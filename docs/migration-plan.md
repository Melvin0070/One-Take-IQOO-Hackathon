...
Use one minute per 2,000 added or deleted text lines in the next section, rounded up and capped at 10 minutes.
Wait longer only when verification is still running.
Commit messages describe the functionality being added.
...
Remove temporary native logging of recognized speech text.
3. Media and capture services: migrate storage, caption generation, audio analysis, reversible edits, event persistence, vision, and guidance, with their existing unit tests and shared string resources.
