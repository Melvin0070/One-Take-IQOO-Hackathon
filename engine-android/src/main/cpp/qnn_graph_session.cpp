...
void throwGraphException(JNIEnv* env, const char* message) noexcept {
    ...
    jclass exceptionClass = env->FindClass("com/onetake/engine/android/QnnGraphException");
    if (!exceptionClass) {
        env->ExceptionClear();
...
jobject makeTensorMetadata(JNIEnv* env, const TensorSpecNative& tensor) {
    jclass tensorClass = env->FindClass("com/onetake/engine/android/QnnNativeTensorSpec");
...
jobject makeGraphMetadata(JNIEnv* env, const GraphSession& session) {
    jclass tensorClass = env->FindClass("com/onetake/engine/android/QnnNativeTensorSpec");
    jclass graphClass = env->FindClass("com/onetake/engine/android/QnnNativeGraphMetadata");
    ...
    const jmethodID tensorConstructor = env->GetMethodID(
        tensorClass, "<init>", "(Ljava/lang/String;I[JI)V");
    const std::string tensorClassName = "Lcom/onetake/engine/android/QnnNativeTensorSpec;";
...
Java_com_onetake_engine_android_QnnGraphNative_open(
...
Java_com_onetake_engine_android_QnnGraphNative_metadata(JNIEnv* env, jobject, jlong handle) {
    try {
#ifndef ONETAKE_QNN_AVAILABLE
...
Java_com_onetake_engine_android_QnnGraphNative_execute(
...
Java_com_onetake_engine_android_QnnGraphNative_close(JNIEnv* env, jobject, jlong handle) {
    try {
#ifndef ONETAKE_QNN_AVAILABLE
    static_cast<void>(handle);
    throwGraphException(env, "QNN graph runtime is unavailable: QAIRT SDK was not configured");
