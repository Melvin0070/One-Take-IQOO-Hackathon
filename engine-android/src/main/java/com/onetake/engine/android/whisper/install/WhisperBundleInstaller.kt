...
class WhisperBundleInstaller private constructor(
     ...
     *
     * The archive is checked against [WhisperBundleSpec.ARCHIVE_SHA256] before
        ...
        try {
            ZipFile(archive).use { zip ->
                ...
                for (entry in entries) {
                    ...
                    if (name == WhisperBundleSpec.ARCHIVE_ROOT) {
    ...
    private fun validateEntryName(name: String) {
        ...
        if (name != WhisperBundleSpec.ARCHIVE_ROOT && !name.startsWith(WhisperBundleSpec.ARCHIVE_ROOT)) {
...
internal class InternalWhisperBundleSpec(
    ...
    companion object {
        val PRODUCTION = InternalWhisperBundleSpec(
            archiveSizeBytes = WhisperBundleSpec.ARCHIVE_SIZE_BYTES,
            archiveSha256 = WhisperBundleSpec.ARCHIVE_SHA256,
            artifacts = WhisperBundleSpec.artifacts.map {
                Artifact(it.name, it.sizeBytes, it.sha256)
            },
            artifactByArchivePath = WhisperBundleSpec.artifactByArchivePath.mapValues { (_, value) ->
